...
input_shape=(256, 1, 512, 512)
output_shape=(2, 1, 1024,1024)
...
def build_engine():
    ...
    conv0_w_t=conv0_w[:,:,np.newaxis,:,:]
    conv_0_bias=params["arg:convolution6_bias"]
    conv0_b=conv_0_bias.asnumpy()

    conv0=network.add_convolution_nd(input=relu0.get_output(0), num_output_maps=2, kernel_shape=(1, 3,3), kernel=conv0_w_t,bias=conv0_b)
    conv0.stride_nd=(1,1,1)
    conv0.padding_nd=(0,1,1)
...
if __name__ == "__main__":
    ...
    with open("model/trt/deconv-activation-conv-3d.trt", "wb") as f:
        f.write(plan)
    f.close()
